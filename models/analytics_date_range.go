// Code generated by Microsoft Kiota - DO NOT EDIT.
// Changes may cause incorrect behavior and will be lost if the code is regenerated.

package models

import (
	i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91 "github.com/microsoft/kiota-abstractions-go/serialization"
	i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e "time"
)

// AnalyticsDateRange defines the inclusive reporting window and comparison window used to calculate Leadping analytics.
type AnalyticsDateRange struct {
	// Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.
	additionalData map[string]any
	// Selects the time interval used to aggregate points in a Leadping analytics trend series.
	bucketSize *int32
	// Date and time when this Leadping analytics date range was end.
	endAt *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time
	// Date and time when this Leadping analytics date range was start.
	startAt *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time
}

// NewAnalyticsDateRange instantiates a new AnalyticsDateRange and sets the default values.
func NewAnalyticsDateRange() *AnalyticsDateRange {
	m := &AnalyticsDateRange{}
	m.SetAdditionalData(make(map[string]any))
	return m
}

// CreateAnalyticsDateRangeFromDiscriminatorValue creates a new instance of the appropriate class based on discriminator value
// returns a Parsable when successful
func CreateAnalyticsDateRangeFromDiscriminatorValue(parseNode i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) (i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.Parsable, error) {
	return NewAnalyticsDateRange(), nil
}

// GetAdditionalData gets the AdditionalData property value. Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.
// returns a map[string]any when successful
func (m *AnalyticsDateRange) GetAdditionalData() map[string]any {
	return m.additionalData
}

// GetBucketSize gets the bucketSize property value. Selects the time interval used to aggregate points in a Leadping analytics trend series.
// returns a *int32 when successful
func (m *AnalyticsDateRange) GetBucketSize() *int32 {
	return m.bucketSize
}

// GetEndAt gets the endAt property value. Date and time when this Leadping analytics date range was end.
// returns a *Time when successful
func (m *AnalyticsDateRange) GetEndAt() *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time {
	return m.endAt
}

// GetFieldDeserializers the deserialization information for the current model
// returns a map[string]func(i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error when successful
func (m *AnalyticsDateRange) GetFieldDeserializers() map[string]func(i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
	res := make(map[string]func(i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error)
	res["bucketSize"] = func(n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
		val, err := n.GetInt32Value()
		if err != nil {
			return err
		}
		if val != nil {
			m.SetBucketSize(val)
		}
		return nil
	}
	res["endAt"] = func(n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
		val, err := n.GetTimeValue()
		if err != nil {
			return err
		}
		if val != nil {
			m.SetEndAt(val)
		}
		return nil
	}
	res["startAt"] = func(n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
		val, err := n.GetTimeValue()
		if err != nil {
			return err
		}
		if val != nil {
			m.SetStartAt(val)
		}
		return nil
	}
	return res
}

// GetStartAt gets the startAt property value. Date and time when this Leadping analytics date range was start.
// returns a *Time when successful
func (m *AnalyticsDateRange) GetStartAt() *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time {
	return m.startAt
}

// Serialize serializes information the current object
func (m *AnalyticsDateRange) Serialize(writer i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.SerializationWriter) error {
	{
		err := writer.WriteInt32Value("bucketSize", m.GetBucketSize())
		if err != nil {
			return err
		}
	}
	{
		err := writer.WriteTimeValue("endAt", m.GetEndAt())
		if err != nil {
			return err
		}
	}
	{
		err := writer.WriteTimeValue("startAt", m.GetStartAt())
		if err != nil {
			return err
		}
	}
	{
		err := writer.WriteAdditionalData(m.GetAdditionalData())
		if err != nil {
			return err
		}
	}
	return nil
}

// SetAdditionalData sets the AdditionalData property value. Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.
func (m *AnalyticsDateRange) SetAdditionalData(value map[string]any) {
	m.additionalData = value
}

// SetBucketSize sets the bucketSize property value. Selects the time interval used to aggregate points in a Leadping analytics trend series.
func (m *AnalyticsDateRange) SetBucketSize(value *int32) {
	m.bucketSize = value
}

// SetEndAt sets the endAt property value. Date and time when this Leadping analytics date range was end.
func (m *AnalyticsDateRange) SetEndAt(value *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time) {
	m.endAt = value
}

// SetStartAt sets the startAt property value. Date and time when this Leadping analytics date range was start.
func (m *AnalyticsDateRange) SetStartAt(value *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time) {
	m.startAt = value
}

type AnalyticsDateRangeable interface {
	i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.AdditionalDataHolder
	i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.Parsable
	GetBucketSize() *int32
	GetEndAt() *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time
	GetStartAt() *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time
	SetBucketSize(value *int32)
	SetEndAt(value *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time)
	SetStartAt(value *i336074805fc853987abe6f7fe3ad97a6a6f3077a16391fec744f671a015fbd7e.Time)
}
